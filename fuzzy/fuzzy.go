...
func (ss stringSource) All() iter.Seq[any] {
...
func FindFromNoSort(pattern string, data Source) Matches {
	...
	for dst := range data.All() {
		dataString := fmt.Sprintf("%s", dst)
		var match Match
		match.Str = dataString
		match.MatchedData = dst
		...
		var last rune
		var lastIndex int
		nextc, nextSize := utf8.DecodeRuneInString(dataString)
		var candidate rune
		var candidateSize int
		for j := 0; j < len(dataString); j += candidateSize {
			...
			if j+candidateSize < len(dataString) {
				if dataString[j+candidateSize] < utf8.RuneSelf { // Fast path for ASCII
					nextc, nextSize = rune(dataString[j+candidateSize]), 1
				} else {
					nextc, nextSize = utf8.DecodeRuneInString(dataString[j+candidateSize:])
				}
			} else {
				nextc, nextSize = 0, 0
			}
		...
		penalty := len(match.MatchedIndexes) - len(dataString)
		match.Score += penalty
